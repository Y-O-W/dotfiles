...
**Landscape scan (do this before drafting a new skill from scratch):** Search for existing skills that already solve this problem, or close to it, so effort isn't spent reinventing something proven. At minimum, check:
- `github.com/anthropics/skills` — Anthropic's official repository; if something here already covers the need, prefer adapting it over writing new
- A community skills directory (e.g. SkillsClaude or similar aggregators) — search by the task's domain/keywords, and weigh results by trust signals (official/verified tags, stars, recency) since community quality varies widely

If a close match turns up, tell the user what you found and ask whether they'd rather adapt the existing skill (layering their own conventions/constraints on top) or proceed with a from-scratch draft anyway. Don't skip this silently — a five-minute search can save an entire draft-test-iterate cycle. If nothing relevant turns up, say so briefly and move on to drafting.
...
### Alternate entry point: usage-log review

Not every improvement pass starts with a fresh eval/iterate cycle the user kicked off. If a `skill-usage-tracker` hook nudge appears in the conversation ("skill X has hit N uses since the last review"), or the user asks you to check how a skill has been performing in real use, read `references/usage-log-review.md` before doing anything else — it covers filtering `~/.claude/skill-usage.jsonl`, sampling the session transcripts it points at for real signal (not just invocation counts), and what patterns are actually worth acting on. Once you've identified something worth fixing, come back here and treat it like any other feedback.
...
Once you stop — whichever of those reasons applies — deploy the result per **Deploying a finished skill locally** further down. An improved skill sitting only in the location you happened to edit it in isn't actually finished yet.
...
- **Deploy the update everywhere it's used, not just back to the source path** — see **Deploying a finished skill locally**. An edit only matters once it reaches every location the user actually invokes the skill from.
...
- `references/usage-log-review.md` — how to review a skill's real-world usage log/transcripts and fold findings into the improvement loop
...
- Deploy the finished skill to every location it's used from — new or updated, see Deploying a finished skill locally
